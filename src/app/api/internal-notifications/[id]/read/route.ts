...
type InternalNotificationReadRouteProps = {
  params: Promise<{
    id: string;
  }>;
};

export async function POST(_request: NextRequest, { params }: InternalNotificationReadRouteProps) {
  ...
  if (existingState) {
    return NextResponse.json({ state: toInternalNotificationReadStateDto(existingState) });
  }
  ...
  if (insertError) {
    return jsonError(
      "Nao foi possivel marcar a notificacao como lida. Tente novamente.",
      500,
      "internal_notification_mark_read_failed",
    );
  }
