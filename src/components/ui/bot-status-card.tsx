...
export function BotStatusCard({ online, name, lastSeen, pending, sentToday, failed }: BotStatusCardProps) {
  ...
  return (
    <article className="relative overflow-hidden rounded-2xl border border-white/75 bg-white/78 p-4 shadow-sm shadow-blue-950/5 ring-1 ring-blue-100/45 backdrop-blur-xl transition duration-200 hover:border-blue-200/80 hover:bg-white/92 hover:shadow-lg hover:shadow-blue-950/10 sm:p-5">
      <div className="absolute right-4 top-4 h-20 w-20 rounded-full bg-sky-200/20 blur-2xl" />
      <div className="relative flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">WhatsApp Bot</p>
      ...
      <div className="relative mt-4 grid gap-2.5 sm:grid-cols-2">
        <div className="rounded-2xl border border-slate-100 bg-slate-50/78 p-3">
          <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.1em] text-slate-500">
            <Clock3 className="h-3.5 w-3.5" /> Ultima conexao
          </div>
          <p className="mt-2 text-sm font-semibold text-slate-950">
            {lastSeen ? new Date(lastSeen).toLocaleString("pt-BR") : "Nao identificada"}
          </p>
        </div>
        <div className="rounded-2xl border border-slate-100 bg-slate-50/78 p-3">
          <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.1em] text-slate-500">
