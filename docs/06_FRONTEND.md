...
- O cliente Windows principal usa WPF em `tools/windows-notifier-app`, tray icon e o endpoint read-only protegido por `WINDOWS_NOTIFIER_TOKEN`; ele nao usa sessao do navegador, nao acessa Supabase diretamente e nao altera estado de leitura. Os pop-ups nao fecham sozinhos, ficam empilhados quando mais de um aviso chega e mantem abertura da central/certificado ao clicar. O build gera a pasta de manutencao `FasaNotifierWpfSetup` e o instalador unico `InstalarNotificadorFasa-Unico.exe`, que limpa a instalacao anterior e deve ser tratado como confidencial por conter `config.local.json` embutido.
