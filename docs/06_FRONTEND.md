...
- A base de notificacoes internas usa `internal_notifications` e `internal_notification_reads` para alimentar o sininho, a central `/notificacoes-internas`, pop-ups do navegador e o cliente Windows WPF em `tools/windows-notifier-app`. As APIs `GET /api/internal-notifications`, `GET /api/internal-notifications/summary`, `GET /api/internal-notifications/windows/summary`, `GET /api/internal-notifications/windows/[id]/certificate-file`, `POST /api/internal-notifications/[id]/read` e `POST /api/internal-notifications/[id]/dismiss` estao prontas. Upload individual e importacao em massa ja registram `certificate_created` e `certificate_updated`.
...
- O cliente Windows principal usa WPF em `tools/windows-notifier-app`, tray icon e endpoints protegidos por `WINDOWS_NOTIFIER_TOKEN`; ele nao usa sessao do navegador, nao acessa Supabase diretamente e nao altera estado de leitura. Os pop-ups nao fecham sozinhos, ficam empilhados quando mais de um aviso chega, mantem abertura da central/certificado ao clicar e baixam PFX direto para `Downloads` somente quando o usuario clica em `Baixar certificado`. O build gera a pasta de manutencao `FasaNotifierWpfSetup` e o instalador unico `InstalarNotificadorFasa-Unico.exe`, que limpa a instalacao anterior e deve ser tratado como confidencial por conter `config.local.json` embutido.
- `next.config.ts` desativa source maps de producao, remove o header `X-Powered-By` e aplica headers de seguranca para reduzir exposicao de codigo, clickjacking, MIME sniffing, permissive referrer e acesso indevido a APIs do navegador.
