...
comment on table public.internal_notifications is 'Fila interna para avisos do painel e futuro app Windows. Nao substitui notification_events e nao dispara WhatsApp.';
comment on column public.internal_notifications.type is 'Tipo de apresentacao da notificacao interna. Nao reutiliza status persistidos do dispatcher WhatsApp.';
comment on column public.internal_notifications.severity is 'Tom visual esperado: info, success, warning ou error.';
comment on column public.internal_notifications.href is 'Caminho interno opcional iniciado por barra. Nao armazena URL externa, token publico ou storage_path.';
comment on column public.internal_notifications.metadata is 'Metadados sem senhas, tokens, service role, storage_path ou payload bruto de provider.';
comment on table public.internal_notification_reads is 'Estado individual de leitura/dispensa de notificacoes internas por usuario autenticado.';
